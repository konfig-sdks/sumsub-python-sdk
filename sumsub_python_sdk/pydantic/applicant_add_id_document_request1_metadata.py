# coding: utf-8

"""
    Sumsub API

    Sumsub is the one verification platform to secure the whole user journey. With Sumsub's customizable KYC, KYB, transaction monitoring and fraud prevention solutions, you can orchestrate your verification process, welcome more customers worldwide, meet compliance requirements, reduce costs and protect your business.  Sumsub has over 2,000 clients in fintech, crypto, transportation, trading, e-commerce and gaming industries including Binance, Wirex, Avis, Bybit, Huobi, Unlimit, Flutter, Kaizen Gaming, and TransferGo.

    The version of the OpenAPI document: 1.0
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel, ConfigDict


class ApplicantAddIdDocumentRequest1Metadata(BaseModel):
    # A document type.
    id_doc_type: str = Field(alias='idDocType')

    # ISO 3166-1 alpha-3 country code.
    country: str = Field(alias='country')

    # `FRONT_SIDE`, `BACK_SIDE` or `null`.
    id_doc_sub_type: typing.Optional[str] = Field(None, alias='idDocSubType')

    # The first applicant name.
    first_name: typing.Optional[str] = Field(None, alias='firstName')

    # The middle applicant name.
    middle_name: typing.Optional[str] = Field(None, alias='middleName')

    # The last applicant name.
    last_name: typing.Optional[str] = Field(None, alias='lastName')

    # Issued date (format `YYYY-mm-dd`, e.g. 2001-09-25).
    issued_date: typing.Optional[str] = Field(None, alias='issuedDate')

    # Valid until date (format `YYYY-mm-dd`, e.g. 2001-09-26).
    valid_until: typing.Optional[str] = Field(None, alias='validUntil')

    # A document number.
    number: typing.Optional[str] = Field(None, alias='number')

    # The applicant date of birth.
    dob: typing.Optional[str] = Field(None, alias='dob')

    # The applicant birthplace.
    place_of_birth: typing.Optional[str] = Field(None, alias='placeOfBirth')

    model_config = ConfigDict(
        protected_namespaces=(),
        arbitrary_types_allowed=True
    )
